...
`0.1.31a0`. Use the release label for tags and GitHub release names, for example
`v0.1.31-alpha`.
...
```powershell
python scripts/prepare_release.py --version 0.1.31a0 --date 2026-07-10 --release-notes-output $env:TEMP\agentledger-0.1.31-alpha-release.md --dry-run
```
...
git pull --ff-only origin master
git status --short --branch
git tag v0.1.31-alpha
git push origin v0.1.31-alpha
...
Create a prerelease from tag `v0.1.31-alpha` using the draft notes generated by
...
gh release create v0.1.31-alpha --repo Martin123132/AgentLedger --title v0.1.31-alpha --notes-file $env:TEMP\agentledger-0.1.31-alpha-release.md --prerelease
```
...
```powershell
git status --short --branch
gh release view v0.1.31-alpha --repo Martin123132/AgentLedger
...
python scripts/release_artifact_doctor.py --version 0.1.31a0 --stage evidence-packet --release-check-json $env:TEMP\agentledger-release-check.json --release-check-summary $env:TEMP\agentledger-release-check-summary.md --release-notes $env:TEMP\agentledger-0.1.31-alpha-release.md --github-release-check-json $env:TEMP\agentledger-github-release-check.json
