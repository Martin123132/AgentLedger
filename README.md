...
```powershell
python -m pip install "git+https://github.com/Martin123132/AgentLedger.git@v0.1.33-alpha"
...
AgentLedger includes a Windows desktop application while keeping the CLI as the
evidence engine. It provides Overview, Capture, History, and Activity views for
day-to-day use. Run it from a source checkout with:
...
`v0.1.33-alpha` release includes the first checked desktop package. Download the
setup or portable ZIP from its GitHub release assets; the unsigned alpha may
...
python scripts/release_command_index.py --version 0.1.33a0 --date 2026-07-19 --format markdown --output $env:TEMP\agentledger-release-command-index.md
...
python scripts/rehearse_release.py --version 0.1.33a0 --date 2026-07-19 --output-dir $env:TEMP\agentledger-release-rehearsal-0.1.33-alpha
python scripts/verify_release_rehearsal.py $env:TEMP\agentledger-release-rehearsal-0.1.33-alpha\release-rehearsal-manifest.json
python scripts/release_artifact_doctor.py --version 0.1.33a0 --stage rehearsal --rehearsal-manifest $env:TEMP\agentledger-release-rehearsal-0.1.33-alpha\release-rehearsal-manifest.json
python scripts/release_rehearsal_receipt.py $env:TEMP\agentledger-release-rehearsal-0.1.33-alpha\release-rehearsal-manifest.json --format markdown --output $env:TEMP\agentledger-release-rehearsal-0.1.33-alpha\release-rehearsal-receipt.md
...
python scripts/release_artifact_doctor.py --version 0.1.33a0 --stage final-notes --release-check-json $env:TEMP\agentledger-release-check.json --release-check-summary $env:TEMP\agentledger-release-check-summary.md
python scripts/finalize_release_notes.py --version 0.1.33a0 --release-check-json $env:TEMP\agentledger-release-check.json --release-check-summary $env:TEMP\agentledger-release-check-summary.md --pr-ci-url https://github.com/Martin123132/AgentLedger/actions/runs/<pr-run> --master-ci-url https://github.com/Martin123132/AgentLedger/actions/runs/<master-run> --release-readiness-url https://github.com/Martin123132/AgentLedger/actions/runs/<release-readiness-run> --tag-ci-url https://github.com/Martin123132/AgentLedger/actions/runs/<tag-run> --merge-sha <merge-sha> --output $env:TEMP\agentledger-0.1.33-alpha-release.md
python scripts/check_github_release.py --version 0.1.33a0 --format json --output $env:TEMP\agentledger-github-release-check.json
python scripts/check_github_release.py --version 0.1.33a0 --format markdown --output $env:TEMP\agentledger-github-release-check.md
python scripts/release_evidence_packet.py --version 0.1.33a0 --release-check-json $env:TEMP\agentledger-release-check.json --release-check-summary $env:TEMP\agentledger-release-check-summary.md --release-notes $env:TEMP\agentledger-0.1.33-alpha-release.md --github-release-check-json $env:TEMP\agentledger-github-release-check.json --output $env:TEMP\agentledger-release-evidence.md --json-output $env:TEMP\agentledger-release-evidence.json
python scripts/release_artifact_doctor.py --version 0.1.33a0 --stage post-release --release-check-json $env:TEMP\agentledger-release-check.json --release-check-summary $env:TEMP\agentledger-release-check-summary.md --release-notes $env:TEMP\agentledger-0.1.33-alpha-release.md
python scripts/post_release_check.py --version 0.1.33a0 --release-check-json $env:TEMP\agentledger-release-check.json --release-check-summary $env:TEMP\agentledger-release-check-summary.md --release-notes $env:TEMP\agentledger-0.1.33-alpha-release.md --output-dir $env:TEMP\agentledger-post-release-0.1.33-alpha
```
...
workspace when it finishes. Pass `-Source "git+https://github.com/Martin123132/AgentLedger.git@v0.1.33-alpha"`
...
It also writes `.agentledger/latest-alpha-packet.json`; use `open-packet` to
find the latest packet again without scrolling terminal output.
...
```powershell
python scripts/release_command_index.py --version 0.1.33a0 --date 2026-07-19
python scripts/check_release_process.py --version 0.1.33a0 --date 2026-07-19
python scripts/release_readiness_report.py --format markdown --output $env:TEMP\agentledger-release-readiness-report.md
python scripts/rehearse_release.py --version 0.1.33a0 --date 2026-07-19 --output-dir $env:TEMP\agentledger-release-rehearsal-0.1.33-alpha
python scripts/verify_release_rehearsal.py $env:TEMP\agentledger-release-rehearsal-0.1.33-alpha\release-rehearsal-manifest.json
python scripts/release_rehearsal_receipt.py $env:TEMP\agentledger-release-rehearsal-0.1.33-alpha\release-rehearsal-manifest.json --format markdown --output $env:TEMP\agentledger-release-rehearsal-0.1.33-alpha\release-rehearsal-receipt.md
python scripts/prepare_release.py --version 0.1.33a0 --date 2026-07-19 --dry-run
python scripts/prepare_release.py --version 0.1.33a0 --date 2026-07-19 --release-notes-output $env:TEMP\agentledger-0.1.33-alpha-release.md
powershell -ExecutionPolicy Bypass -File scripts/release-check.ps1
powershell -ExecutionPolicy Bypass -File scripts/release-check.ps1 -RequireCleanGit
powershell -ExecutionPolicy Bypass -File scripts/release-dry-run.ps1
python scripts/release_notes.py --version 0.1.33a0 --check
python scripts/release_notes.py --version 0.1.33-alpha --output $env:TEMP\agentledger-0.1.33-alpha-release.md
python scripts/release_notes.py --version 0.1.33a0 --notes-file $env:TEMP\agentledger-0.1.33-alpha-release.md --check-publish-ready
```
...
`support-packet` prints the exact alpha support report checklist, a sanitized
Markdown issue/comment body with `--format markdown`, and a JSON contract with
`--format json` without writing files, copying evidence, or including local
paths by default.
...
The latest packet pointer is saved as `.agentledger/latest-alpha-packet.json`,
and `agentledger open-packet --out .agentledger` prints the issue, Markdown, and
JSON packet paths again.
