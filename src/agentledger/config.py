...
POLICY_PRESETS: dict[str, dict[str, str]] = {
    "solo": {
        "description": "Default local solo-builder policy with summary privacy and warning-level dirty state.",
        "text": STARTER_CONFIG_TEXT,
    },
    "agency": {
        "description": "Agency delivery policy for reviewed client work with tighter changed-file limits.",
        "text": """# AgentLedger agency delivery policy.
# Use for AI-assisted work that needs a reviewer-friendly receipt before handoff.
privacy_mode = "summary"
out = ".agentledger"
repomori = false
jester = false
...
# Review policy for agentledger receipt/check.
check_require_tests = true
check_dirty = "warn"
check_max_changed_files = 15
check_allow_warnings = false
""",
    },
    "team-strict": {
        "description": "Strict team policy: require tests, block dirty final state, and fail on warnings.",
        "text": """# AgentLedger strict team policy.
# Use for internal teams that want receipts to fail until the repo is clean.
privacy_mode = "summary"
out = ".agentledger"
repomori = false
jester = false
...
""",
    ...
    "client-handoff": {
        "description": "Client handoff policy with summary privacy, required tests, and explicit warning review.",
        "text": """# AgentLedger client handoff policy.
# Use when receipts may be shown outside the delivery machine after review.
privacy_mode = "summary"
out = ".agentledger"
repomori = false
jester = false
...
def preset_names() -> tuple[str, ...]:
    return tuple(POLICY_PRESETS.keys())


def preset_text(name: str) -> str:
    try:
        return POLICY_PRESETS[name]["text"]
    except KeyError as exc:
        raise ConfigError(f"Unknown policy preset: {name}") from exc


def preset_description(name: str) -> str:
    try:
        return POLICY_PRESETS[name]["description"]
    except KeyError as exc:
        raise ConfigError(f"Unknown policy preset: {name}") from exc
